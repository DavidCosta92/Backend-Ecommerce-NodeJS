...
export const authGithub = passport.authenticate('github', { session: true , scope: ['user:email'] })  // PARA JWT, AL ELIMINAR TODO LO DE SESSION, ESTAS CONFIG DEBERAN LLEVAR session en false ('local', { session:false ,failWithError: true }) 
export const callbackAuthGithub = passport.authenticate('github', { session: true ,  failWithError: true })   // PARA JWT, AL ELIMINAR TODO LO DE SESSION, ESTAS CONFIG DEBERAN LLEVAR session en false ('local', { session:false ,failWithError: true }) // { failWithError: true } => en caso de que falle deberia llamar al next()
